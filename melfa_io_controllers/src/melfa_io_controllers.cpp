...
controller_interface::InterfaceConfiguration MelfaGPIOController::command_interface_configuration() const
{
  ...
  config.names.insert(config.names.end(), plc_link_io_command_interfaces_.begin(),
                      plc_link_io_command_interfaces_.end());
...
controller_interface::return_type MelfaGPIOController::update(const rclcpp::Time& /*time*/,
                                                              const rclcpp::Duration& /*period*/)
{
  ...
  auto updateGPIO = [&](melfa_msgs::msg::GpioState& gpio_msg, const char* io_interface_name, GpioIdentifier identifier,
    ...
    gpio_msg.bit_send_type = (state_interfaces_.at(identifier + 2).get_value() == 0) ?
                                 "MXT_IO_NULL" :
                                 ((state_interfaces_.at(identifier + 2).get_value() == 1) ? "MXT_IO_OUT" : "MXT_IO_IN");
  ...
  ctrl_type_msg.controller_type =
      (state_interfaces_.at(GpioIdentifier::ctrl_type).get_value() == 1.0) ?
          "R" :
          ((state_interfaces_.at(GpioIdentifier::ctrl_type).get_value() == 2.0) ? "Q" : "D");
...
MelfaGPIOController::on_configure(const rclcpp_lifecycle::State& /*previous_state*/)
{
  /**
  ...
  try
  {
    ...
    hand_io_state_publisher_ =
        get_node()->create_publisher<melfa_msgs::msg::GpioState>("~/hand_io_state", rclcpp::SystemDefaultsQoS());

    plc_link_io_state_publisher_ =
        get_node()->create_publisher<melfa_msgs::msg::GpioState>("~/plc_link_io_state", rclcpp::SystemDefaultsQoS());

    safety_io_state_publisher_ =
        get_node()->create_publisher<melfa_msgs::msg::GpioState>("~/safety_io_state", rclcpp::SystemDefaultsQoS());

    io_unit_state_publisher_ =
        get_node()->create_publisher<melfa_msgs::msg::GpioState>("~/io_unit_state", rclcpp::SystemDefaultsQoS());

    misc1_io_state_publisher_ =
        get_node()->create_publisher<melfa_msgs::msg::GpioState>("~/misc1_io_state", rclcpp::SystemDefaultsQoS());

    misc2_io_state_publisher_ =
        get_node()->create_publisher<melfa_msgs::msg::GpioState>("~/misc2_io_state", rclcpp::SystemDefaultsQoS());

    misc3_io_state_publisher_ =
        get_node()->create_publisher<melfa_msgs::msg::GpioState>("~/misc3_io_state", rclcpp::SystemDefaultsQoS());

    controller_type_publisher_ =
        get_node()->create_publisher<melfa_msgs::msg::ControllerType>("~/controller_type", rclcpp::SystemDefaultsQoS());

    control_mode_publisher_ =
        get_node()->create_publisher<melfa_msgs::msg::ControlMode>("~/io_control_mode", rclcpp::SystemDefaultsQoS());

    gpio_command_subscriber_ = get_node()->create_subscription<melfa_msgs::msg::GpioCommand>(
        "~/gpio_command", rclcpp::SystemDefaultsQoS(),
        [this](const melfa_msgs::msg::GpioCommand::SharedPtr gpio_msg) { commandGPIOCallback(gpio_msg); });

    configure_gpio_srv_ = get_node()->create_service<melfa_msgs::srv::GpioConfigure>(
        "~/configure_gpio",
        std::bind(&MelfaGPIOController::configGpio, this, std::placeholders::_1, std::placeholders::_2));

    configure_mode_srv_ = get_node()->create_service<melfa_msgs::srv::ModeConfigure>(
        "~/configure_mode",
        std::bind(&MelfaGPIOController::configControlMode, this, std::placeholders::_1, std::placeholders::_2));
  }
  catch (...)
  {
...
bool MelfaGPIOController::configGpio(melfa_msgs::srv::GpioConfigure::Request::SharedPtr req,
                                     melfa_msgs::srv::GpioConfigure::Response::SharedPtr res)
{
  /**
   ...
   */
  auto processIO = [&](auto& command_interfaces_, melfa_msgs::srv::GpioConfigure::Request::SharedPtr req,
                       GpioIdentifier identifier, const std::array<uint16_t, 3>& mode_values, long int ctrl_limit) {
    res->success = true;
  ...
  else if ((req->mode == req->SET_READ_IN && ((req->bitid >= ctrl_limits[4] && req->bitid <= ctrl_limits[5]) ||
                                              (req->bitid >= ctrl_limits[6] && req->bitid <= ctrl_limits[7]))) ||
           (req->mode == req->SET_READ_OUT && ((req->bitid >= ctrl_limits[8] && req->bitid <= ctrl_limits[9]) ||
...
void MelfaGPIOController::commandGPIOCallback(const melfa_msgs::msg::GpioCommand::SharedPtr io_cmd_)
{
  ...
  auto commandGPIO = [&](GpioIdentifier identifier, long int ctrl_limit) {
    auto cmdType = [](const std::string& typeSendRecv) -> double {
      return (typeSendRecv == "MXT_IO_NULL") ? 0.0 :
             (typeSendRecv == "MXT_IO_OUT")  ? 1.0 :
                                            ...
                                            melfa_msgs::srv::ModeConfigure::Response::SharedPtr res)
{
   ...
   */
  std::string control_mode_binary_ = std::to_string(req->misc3_io_interface) + std::to_string(req->misc2_io_interface) +
                                     std::to_string(req->misc1_io_interface) + std::to_string(req->io_unit_interface) +
  ...
  res->success =
      (control_mode_int_ <= 0b1111111) ?
          (command_interfaces_[GpioIdentifier::control_mode_io].set_value(static_cast<double>(control_mode_int_)),
           true) :
          false;
...
controller_interface::CallbackReturn MelfaGPIOController::on_activate(const rclcpp_lifecycle::State& /*previous_state*/)
{
  /**
...
controller_interface::CallbackReturn
MelfaGPIOController::on_deactivate(const rclcpp_lifecycle::State& /*previous_state*/)
{
  /**
