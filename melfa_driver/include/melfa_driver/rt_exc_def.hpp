...
{
  /**
   * @brief Command. MXT_CMD_NULL for NULL command. MXT_CMD_MOVE for Move command. MXT_CMD_END to end real time external
   * control function in robot controller
   */
  uint16_t Command;
  /**
  ...
  /**
   * @brief Send input/output signal data designation. MXT_IO_OUT for output signal. MXT_IO_NULL for input signal. When
   * used in MXTsend, designate signal type of data sent. When used in MXTrecv, indicate signal type of data recevied.
   */
  uint16_t SendIOType;
  /**
   * @brief Return input/output signal data designation. MXT_IO_OUT for output signal. MXT_IO_NULL for input signal.
   * When used in MXTsend, designate signal type of data received. Not used in MXTrecv
   */
  uint16_t RecvIOType;
  /**
   * @brief Head bit Number. When used in MXTsend, designate head bit of data sent. When used in MXTrecv. indicate head
   * bit of data received
   */
  uint16_t BitTop;
  /**
   * @brief Transmission bit mask pattern designation (0x0000-0xffff). When used in MXTsend, bit mask for data sent.
   * BitMask AND IoData = actual data sent. Not used in MXTrecv
   */
  uint16_t BitMask;
  /**
