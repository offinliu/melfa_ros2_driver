...
rtexc::rtexc(float custom_cycle_time)
  : MXTsend{}, MXTrecv{}, console_msg{}, sendText{}, recvText{}, packet_recv_lost(0), counter_(0)
{
#ifdef _WIN32
...
#if __linux__
  (robot_ip).destAddr =
      inet_addr((robot_ip).dst_ip_address.c_str());  // get IPv4 network address in standard dot format.
#endif
#if _WIN32
  (robot_ip).destAddr =
      inet_addr((robot_ip).dst_ip_address.c_str());  // get IPv4 network address in standard dot format.
...
int rtexc::send_packet_()
{
  ...
  if (RT_API_DEBUG_MODE)
  {
    ...
    sprintf(console_msg, "Sending to: destAddr=%d, destSocket=%d, dst_ip_address = %s, port= %d.\n", robot_ip.destAddr,
            robot_ip.destSocket, robot_ip.dst_ip_address.c_str(), robot_ip.port);
#elif _WIN32
  ...
  size = sendto(robot_ip.destSocket, sendText, sizeof(MXTCMD), NO_FLAGS_SET, (struct sockaddr*)&(robot_ip.destSockAddr),
                sizeof((robot_ip.destSockAddr)));
